...
As the **Data Engineer**, your task is to build the complete streaming analytics pipeline. You begin by creating a dedicated **Fabric Workspace** and an **Eventhouse** to store real-time event data. To ensure interoperability with other Fabric experiences, you enable OneLake Availability.

Next, you configure an **Eventstream** with a custom endpoint that receives the synthetic events generated by the notebook. Within the Eventstream, you separate click events and impression events into different destinations so they can be stored independently in the Eventhouse.
...
Finally, you create a **Real-Time Dashboard** to monitor streaming metrics, enable automatic refresh so the visuals stay up to date, and configure a Data Activator rule that can trigger actions when specified thresholds are reached.
...
1. You will be navigated back to the **Microsoft** **PowerBI Home page**. Click on the **PowerBI (1)** icon on the bottom left corner, and select **Fabric (2)**. 
